...
public class Mandelbrot {
    ...
    private static int limit = 255;	// Maximum iterations for membership testing
    private static int rows = 100;	// Horizontal size (in pixels) of drawing
    private static int columns = 150;	// Vertical size (in pixels) of drawing
    private static int pixels = 500;
    private static double size = 2.0;

    public Mandelbrot(double centerReal, double centerImaginary, int rows, int columns, int iterations) {
        this.rows = rows;
    ...
    public void draw() {
        ...
        for (int row = 0; row < rows; row++) {
            for (int col = 0; col < columns; col++) {
                ...
                if (isMember(z) == limit) {//if it didn't diverge
                    StdDraw.point(re, im);
                }
            }
        }
